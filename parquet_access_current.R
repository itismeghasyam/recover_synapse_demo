...
########
#### Current version of the external parquet dataset (syn52506068)
########
sts_token <- synapser::synGetStsStorageToken(entity = 'syn52506068', # sts enabled destination folder
...
base_s3_uri_external <- paste0(sts_token$bucket, "/", sts_token$baseKey)
...
for (dataset in parquet_datasets_external) {
  if (grepl('recover-main-project/main/parquet/', dataset$path, perl = T, ignore.case = T)) {
    i <- i+1
    cat(i)
    cat(":", dataset$path, "\n")
...
valid_paths_ext_df <- valid_paths %>% 
  ...
  dplyr::rowwise() %>% 
  dplyr::mutate(datasetType = str_split(parquet_path_external,'/')[[1]][4]) %>%
  dplyr::ungroup()
...
##
write.csv(out.df, 'test_upload_current.csv')
...
thisFileName <- "parquet_access_current.R" # location of file inside github repo
thisRepo <- githubr::getRepo(repository = "itismeghasyam/recover_synapse_demo",
                             ref="branch",
                             refName="main")
thisFile <- githubr::getPermlink(repository = thisRepo, repositoryPath=thisFileName) # Get exact commit at the time of running this code
...
OUTPUT_FILE <- "test_upload_current.csv" # name your file
